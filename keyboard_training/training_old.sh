...
func_make_rnd_chs_arr() {
	...
	local offset=0				#~ offset index of chs_arr

	(( ${#chs_arr[@]} != 0 )) && offset=$((${#chs_arr[@]} - 1))

	for (( i = offset; i < (offset + 5); i++ )); do
...
func_app_one_more_ch_in_rnd() {
	local missed_char="$1"				#~ char which user miss
	local ch_arr_len=$((${#chs_arr[@]}-1))		#~ length of "chars arr"

	rnd_chs[${#rnd_chs[@]}]="$missed_char"		#~ append missed char to random chars list
	chs_arr[$ch_arr_len]="$missed_char"		#~ append missed char to end of chars array

	func_choose_one_rnd_ch $(($ch_arr_len + 1))
	chs_arr[$ch_arr_len+2]=" "			#~ chs_arr must end on SPACE
}
